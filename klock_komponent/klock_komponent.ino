/*
Name: övningsprojekt
Author: Victor Huke
Date: 2025-10-8
Description: This project uses a ds3231 to measure time and displays the time to an 1306 oled display, 
Further, it measures temprature and displays a mapped value to a 9g-servo-motor
*/

// Include Libraries
#include <RTClib.h>
#include <Wire.h>
#include <U8glib.h>
#include <Servo.h>
#include <LedControl.h>

// Init constants

// Init global variables

// Construct objects
RTC_DS3231 rtc;
U8GLIB_SSD1306_128X64 u8g(U8G_I2C_OPT_NO_ACK);
Servo myServo;
LedControl lc = LedControl(7, 6,5, 1);

// startar komponenterna och sätter display inställningar
void setup() {
  Serial.begin(9600);
  Wire.begin();
  rtc.begin();
  myServo.attach(9);

  lc.shutdown(0, false);
  lc.setIntensity(0, 1); 
  lc.clearDisplay(0);    

  rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
}

void loop() {
 // tar tempen och tiden och skickar till oled write
  float temp = getTemp();
  String timeNow = getTime();
  oledWrite(timeNow,temp, getDate());
  // skriver ut tempen till servon
  servoWrite(temp);
  // tar tiden från rtc och omvandlar till en integer
  int hour = rtc.now().hour();
  int minute = rtc.now().minute();
  int second = rtc.now().second();
 // sätter tiden på rätt position på displayen
 // /10 för att få tiotalet och sedan % för att få entalet 
  lc.setDigit(0, 7, hour / 10, false); 
  lc.setDigit(0, 6, hour % 10, false);   
  lc.setDigit(0, 5, minute / 10, false);  
  lc.setDigit(0, 4, minute % 10, false);  
  lc.setDigit(0, 3, second / 10, false);  
  lc.setDigit(0, 2, second % 10, false);
// printar ut till serial monitor
  Serial.print("Tid: ");
  Serial.print(getTime());
  Serial.print("  Temp: ");
  Serial.println(temp);
  
  delay(200);
}



/*
*This function reads time from an ds3231 module and package the time as a String
*Parameters: Void
*Returns: time in hh:mm:ss as String
*/
// gets the time 
String getTime() {
  DateTime now = rtc.now();
  return String(now.hour()) + ":" + String(now.minute()) + ":" + String(now.second());
  
}
// gets the date
String getDate() {
  DateTime now = rtc.now();
  return String(now.year()) + "/" + String(now.month()) + "/" + String(now.day());
  
}
/*
This function takes temprature from ds3231 and returns as a float
*Parameters: Void
*Returns: temprature as float
*/
float getTemp() {
  return rtc.getTemperature();
}
/*
This function takes a string and draws it to an oled display
*Parameters: - text: String to write to display
*Returns: void
*/
// takes time temp and date, and draws it on the oled display
void oledWrite(String text, float temp, String text2) {
  u8g.setFont(u8g_font_unifont);
  u8g.firstPage();
  do {
    u8g.drawStr(0, 10, text.c_str());
    String tempStr = "Temp: " + String(temp, 1) + "C";  
    u8g.drawStr(0, 50, tempStr.c_str());
    u8g.drawStr(0, 30, text2.c_str());
  } while (u8g.nextPage());
}
// servo som visar temp
void servoWrite(float value) {
  // Mappa temp (0°C–40°C) till servo (0°–180°)
  int angle = map((int)value, 0, 40, 0, 180);
  angle = constrain(angle, 0, 180);  // begränsa till giltig vinkel
  myServo.write(angle);
}
